...
- **OE-2 DONE** — `ExpressOnboarding.tsx` (client) + store partage `onboardingExpressStore.ts` : 3 etapes empilees numerotees (Confirmez votre commerce -> `/onboarding/secteur` ; Personnalisez votre carte (30s) -> `/dashboard/studio?express=1` ; Distribuez votre QR -> `/dashboard/card`). Etat reel par etape (A faire / En cours / Fait + coche verte), zero case manuelle : etape 1 = `loyalty_type` pose, 2 = `reward_label` pose, 3 = >=1 carte distribuee OU `halo_poster_done` (QR telecharge, signal deja ecrit par EnrollmentQR/QrPosterButton). Deux sorties posant `halo_onboarding_dismissed=1` : lien "Tout est pret, masquer ce guide" + skip "Decouvrir ma carte sans guide". Verifie : `/dashboard/card` a bien un bouton telecharger (QR PNG + affichette PDF). Tests `ExpressOnboarding.test.tsx` : **8/8 verts**. Gate : tsc clean, vitest **964** (+8).

- **OE-3 DONE** — `dashboard/page.tsx` (server, `currentMerchantId()`+`supabaseAdmin`) lit `created_at/loyalty_type/reward_label`, compte `loyalty_cards` (best-effort, tenancy `.eq("merchant_id")`), calcule `isFirstRunMerchant` + etats d'etapes, et delegue a `<OnboardingGate>` (client). **Regle CHEF appliquee** : plein ecran Express RESERVE aux comptes non configures (`loyalty_type` OU `reward_label` manquant) ; des que les DEUX sont poses -> Comptoir DIRECT meme < 24 h. Tant que configure + aucune carte distribuee (`cardsCount===0`, premier passage) -> petit bandeau non bloquant `<ComptoirDistributeBanner>` (-> `/dashboard/card`) en haut du Comptoir via nouveau slot optionnel `banner` de `<ComptoirHome>` (additif : rendu identique sans bandeau, test Comptoir 5/5 intact). Guide masquable (`halo_onboarding_dismissed`). Tests `OnboardingGate.test.tsx` **8/8**. Gate : tsc clean, vitest **972**.
