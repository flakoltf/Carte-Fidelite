...
| MECANIQUE-POINTS | `agent/mecanique-points` | `4c37aa5` | **M1+M2+M3 PASS (gate vert 805/805)** | RPC livrée — reste branchement route `/api/scan` (M4 ?) |
...
- **Périmètre M1→M3 terminé.** Reste (M4 ?, à cadrer par le CHEF) : brancher `/api/scan/route.ts` sur `scan_increment_amount` quand `program.type === "amount_points"` (lire le montant, passer `pointsPerChf`/`maxPointsPerScan`/`rewardThreshold`, mapper le jsonb) + UI (l'`<AmountPad>` d'UX-COMPTOIR est prêt côté front). Non démarré : le message d'assignation M3 était tronqué côté route/tests — j'attends le cadrage.
