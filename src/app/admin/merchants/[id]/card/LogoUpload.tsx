...
export default function LogoUpload({ merchantId, onUploaded }: LogoUploadProps) {
    ...
    return (
      <div className="space-y-2">
        <div
          ...
          onDrop={handleDrop}
          className={`flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-2xl px-6 py-8 cursor-pointer transition-all text-center ${
            dragOver
              ? 'border-halo bg-halo/5'
              : 'border-halo/30 hover:border-halo hover:bg-halo/5'
          }`}
        >
          <Upload className="w-8 h-8 text-halo/60" />
          <p className="text-sm font-medium text-galet-ink">
            Glisser un logo ou{' '}
            <span className="text-halo underline underline-offset-2">parcourir</span>
          </p>
          <p className="text-xs text-galet">PNG, JPG, WebP · tailles Apple + Google générées automatiquement</p>
        </div>
  ...
  if (state === 'cropping') {
    return (
      <div className="space-y-3">
        <p className="text-xs font-medium text-galet-ink">Recadrer le logo</p>
