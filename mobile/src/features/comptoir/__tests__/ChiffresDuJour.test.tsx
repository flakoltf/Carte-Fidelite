...
describe("ChiffresDuJour", () => {
  it("affiche les scans du jour, les cartes actives et les récompenses dues", async () => {
    await render(<ChiffresDuJour stats={{ scansToday: 17, activeCards: 128, rewardsDue: 4 }} chargement={false} />);
  ...
  it("garde les derniers chiffres connus pendant un rafraîchissement", async () => {
    await render(<ChiffresDuJour stats={{ scansToday: 3, activeCards: 9, rewardsDue: 1 }} chargement />);
  ...
  it("affiche zéro plutôt que rien un jour sans scan", async () => {
    await render(<ChiffresDuJour stats={{ scansToday: 0, activeCards: 0, rewardsDue: 0 }} chargement={false} />);
